...
namespace Swordfish.NET.Charts {
  ...
  public static class ChartUtilities {
    ...
    public static void CopyChartToFile(string filename, FrameworkElement plotToCopy, ChartControl chartControl, double width, double height) {
      plotToCopy = plotToCopy ?? chartControl;
      CopyFrameworkElementToPNGFile(plotToCopy, width, height, filename, 96);
    }
    ...
    public static async Task<ChartPrimitiveXY> AddPrimitive(this ChartControl factory, string label, IEnumerable<double> pointsX, IEnumerable<double> pointsY, Color color, bool reducePoints = false) {
      var retVal = await Task.Run(() => {
        var pointsXY = pointsX.Zip(pointsY, (x, y) => new Point(x, y));
        return factory.AddPrimitive(label, pointsXY, color, reducePoints);
      });
      return retVal;
    }
    ...
    public static IEnumerable<Point> ReducePoints(IEnumerable<Point> points) {
      List<Point> newPoints = new List<Point>();
      Point? startPoint = null;
      Point? midPoint = null;
      foreach (Point? endPoint in points) {
        // Check if we have added a point for the start, if not then add it and continue
        if (!startPoint.HasValue) {
          startPoint = endPoint;
          newPoints.Add(startPoint.Value);
          continue;
        } else if (!midPoint.HasValue) {
          // Skip this iteration if the midpoint hasn't been assigned yet
        } else {
          // Start, and midpoints have been assigned, now test if the midpoint lies between start and end
          var dSM = midPoint.Value - startPoint.Value;
          var dSE = endPoint.Value - startPoint.Value;

          if (dSM.LengthSquared == 0) {
            // Skip midpoint as it is on the start point, Go to next
          } else if (dSM.LengthSquared > dSE.LengthSquared) {
            // If we've doubled back then add the midpoint
            newPoints.Add(midPoint.Value);
            startPoint = midPoint;
          } else if (Math.Abs(Vector.AngleBetween(dSM, dSE)) > 0.000001) {
            // line bends, add the mid point
            newPoints.Add(midPoint.Value);
            startPoint = midPoint;
          }
        }
    ...
    public static Stream CopyFrameworkElementToStream(FrameworkElement copyTarget,
      ...
      if(dpi == null) {
        ...
        // JCSTODO: To avoid using multiple RenderTargetBitmap use VisualBrush instead of ImageBrush
