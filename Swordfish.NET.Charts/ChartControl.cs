...
namespace Swordfish.NET.Charts {
  public class ChartControl : DockPanel {
    ...
    public BitmapSource Backdrop {
      get {
        TransformedBitmap invertedBitmapSource = _plotRenderer.BackgroundImage as TransformedBitmap;
        if (invertedBitmapSource == null) {
          return null;
        }
        BitmapSource rawImage = invertedBitmapSource.Source as BitmapSource;
        return rawImage;
      }
      set {
        // Create the new BitmapSource that will be used to scale the size of the source.
        TransformedBitmap invertedBitmapSource = new TransformedBitmap();

        // BitmapSource objects like TransformedBitmap can only have their properties 
        // changed within a BeginInit/EndInit block.
        invertedBitmapSource.BeginInit();
        ...
        invertedBitmapSource.Source = value;

        // Invert the source image, because that's what the chart uses
        invertedBitmapSource.Transform = new ScaleTransform(1, -1);
        invertedBitmapSource.EndInit();


        _plotRenderer.BackgroundImage = invertedBitmapSource;
    ...
    private void AddChildControls() {
      ...
      _xGridLineLabels = new Rectangle() {
        ...
        Fill = new SolidColorBrush(Color.FromArgb(127, 255, 255, 255)),
        IsHitTestVisible = false
      };
      ...
      Grid.SetColumn(_gridLineCanvas, 2);
      Grid.SetRow(_gridLineCanvas, 2);
      _host.Children.Add(_gridLineCanvas);
